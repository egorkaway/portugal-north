import type { MobileMessages } from '@/i18n/types';

export const gl: MobileMessages = {
  lang: {
    label: 'Idioma',
    en: 'English',
    pt: 'Português',
    es: 'Español',
    gl: 'Galego',
    ca: 'Català',
    uk: 'Українська',
    ru: 'Русский',
  },
  nav: {
    home: 'Inicio',
    map: 'Mapa',
    trip: 'Viaxe',
    rankings: 'Rankings',
    tickets: 'Billetes',
    lines: 'Liñas',
    line: 'Liña',
    station: 'Estación',
    privacy: 'Privacidade',
    back: 'Volver',
  },
  common: {
    continue: 'Continuar',
    requesting: 'Solicitando…',
    skip: 'Agora non',
    open: 'Abrir',
    view: 'Ver',
    delete: 'Eliminar',
    cancel: 'Cancelar',
    yes: 'Si',
    no: 'Non',
    goBack: 'Volver atrás',
    build: 'Compilación {{build}}',
  },
  brand: {
    name: 'VeryStays',
    subtitle: 'Hubs de viaxe na península ibérica',
  },
  home: {
    searchPlaceholder: 'Buscar estación ou liña...',
    searchClear: 'Borrar busca',
    sortByDistance: 'Ordenar por distancia',
    sortedByDistance: 'Ordenado por distancia',
    locating: 'Localizando...',
    nearMe: 'Preto de min',
    allVotes: 'Todos os votos',
    upvoted: 'Voto positivo',
    downvoted: 'Voto negativo',
    notVoted: 'Sen voto',
    allVisits: 'Todas as visitas',
    visited: 'Visitada',
    notVisited: 'Non visitada',
    stationCount_one: '{{count}} estación',
    stationCount_other: '{{count}} estacións',
    showingCount: 'mostrando {{count}}',
    noResults: 'Ningunha estación coincide coa busca.',
    noPhoto: 'Sen foto',
    markVisited: 'Marcar como visitada',
    visitedBadge: 'Visitada',
    browseLines: 'Ver liñas de tren',
  },
  station: {
    notFound: 'Estación non atopada',
    markVisited: 'Marcar como visitada',
    visited: 'Visitada',
    liveDepartures: 'Saídas en directo',
    reliabilityTitle: 'Fiabilidade de puntualidade',
    reliabilityBody: 'Baseada nos atrasos acumulados das saídas en tempo real.',
    budgetStays: 'Aloxamentos con prezos razoables preto',
    hotelsIntro: 'Hoteis económicos seleccionados a poucos minutos a pé da estación.',
    openAppleMaps: 'Abrir en Apple Maps',
    searchBooking: 'Buscar en Booking',
    photoQuestion: 'A foto da parte superior desta páxina representa {{name}}?',
    photoYes: 'Si',
    photoNo: 'Non',
    photoHint: 'Axúdanos a elixir mellores fotos das estacións.',
    photoCommunity: 'Comunidade: {{summary}}',
    photoCreditBy: 'Foto de {{author}}',
    ratePhotoA11y: 'Valorar se a foto da estación de arriba representa a estación',
    photoYesA11y: 'Si, a foto representa a estación',
    photoNoA11y: 'Non, a foto non representa a estación',
    away: 'a {{distance}}',
    longDistanceNearby: 'Paradas de longa distancia máis próximas',
    longDistanceIntro:
      'Esta parada ten só servizo rexional ou urbano. Para trens Alfa Pendular ou {{icService}}, proba estas estacións:',
  },
  lines: {
    intro: 'Explora as liñas ferroviarias de Portugal — e as liñas transfronteirizas nos nosos datos. Cada liña lista as estacións e os servizos que paran alí.',
    portugalHeading: 'Liñas ferroviarias de Portugal',
    spainHeading: 'Liñas transfronteirizas e españolas',
    stationCount_one: '{{count}} estación',
    stationCount_other: '{{count}} estacións',
    servicesNote: 'Os tipos de servizo son os rexistrados para cada estación nos nosos datos e poden non reflectir todos os trens todos os días.',
    stationsHeading: 'Estacións nesta liña',
    alsoOn: 'Tamén en',
    emptyLine: 'Aínda non hai estacións rexistradas para esta liña.',
    orderingNote: 'As estacións listanse en orde xeográfica aproximada ao longo da liña, non na secuencia oficial de paradas.',
    viewAllLines: 'Ver todas as liñas',
    notFound: 'Liña non atopada',
  },
  departures: {
    unavailable: 'Saídas en directo temporalmente non dispoñibles',
    none: 'Non hai saídas nas próximas horas.',
    train: 'tren',
    platform: 'andeén',
    delayMin: '+{{minutes}} min',
    take: 'Vou coller',
    taking: 'A coller',
    loadMore: 'Mostrar máis trens',
    loadingMore: 'A cargar…',
  },
  arrivals: {
    title: 'Próximas chegadas',
    none: 'Non hai chegadas nas próximas horas.',
    fromOrigin: 'desde {{origin}}',
    continuesTo: 'desde {{origin}} → {{destination}}',
    terminates: 'termina aquí',
    meet: 'Recibir',
    meeting: 'Recibindo',
  },
  countdown: {
    leavingNow: 'Saíndo agora',
    leavesIn: 'Sae en {{minutes}} min',
    leavesInHours: 'Sae en {{hours}}h {{minutes}}m',
    leavesInHoursOnly: 'Sae en {{hours}}h',
    arrivingNow: 'A chegar agora',
    arrivesIn: 'Chega en {{minutes}} min',
    arrivesInHours: 'Chega en {{hours}}h {{minutes}}m',
    arrivesInHoursOnly: 'Chega en {{hours}}h',
    minutesAgo: 'hai {{minutes}} min',
    minutesAgoHours: 'hai {{hours}}h {{minutes}}m',
    minutesAgoHoursOnly: 'hai {{hours}}h',
  },
  airport: {
    title: 'Conexións aéreas',
    intro: '{{destinations}} destinos con voos directos recollidos ata agora.',
    legend: 'As cores das liñas mostran a frecuencia de cada ruta nas mostras deste período:',
    legendBusy: '5+ voos',
    legendModerate: '3–4 voos',
    legendLight: '1–2 voos',
    flights_one: '{{count}} voo',
    flights_other: '{{count}} voos',
  },
  trip: {
    emptyTitle: 'Sen viaxe activa',
    emptyBody:
      'Abre unha estación, preme Vou coller nunha saída ou chegada que continúa, ou Recibir nunha chegada final, e a conta atrás aparecerá aquí.',
    departed: 'Partiu',
    departedAt: 'Partiu ás {{time}}',
    expectedDeparture: 'Saída prevista ás {{time}}',
    stopTracking: 'Deixar de seguir',
    upcomingStops: 'Próximas paradas',
    loadingStops: 'A cargar paradas…',
    stopsUnavailable: 'Lista de paradas non dispoñible agora mesmo.',
    pastTrips: 'Viaxes anteriores',
    historyDeleteConfirmTitle: 'Eliminar esta viaxe?',
    historyDeleteConfirmBody:
      'Quitar {{train}} ({{origin}} → {{destination}}) do historial? Esta acción non se pode desfacer.',
    origin: 'Orixe',
    finalStop: 'Final',
    delayMin: '+{{minutes}} min',
    platform: 'Andeén {{platform}}',
    refreshWidget: 'Actualizar widget',
    shareA11y: 'Compartir viaxe',
    shareTrip: 'Compartir viaxe',
    shareFailedTitle: 'Non se puido compartir',
    shareFailedBody: 'Algo saíu mal ao preparar a imaxe da viaxe.',
    sharingUnavailable: 'Compartir non está dispoñible neste dispositivo.',
  },
  widget: {
    title: 'Widget da pantalla de inicio',
    howTo: 'Engade Conta atrás do tren desde a galería de widgets:',
    step1: 'Mantén premida a pantalla de inicio e toca +',
    step2: 'Busca VeryStays',
    step3: 'Engade Conta atrás do tren',
  },
  mapa: {
    title: 'Vas planear un Camiño?',
    body: 'GetMapa axúdache a explorar rutas a pé pola Iberia.',
    cta: 'Obter GetMapa',
  },
  sovnik: {
    title: 'Aprende linguas ibéricas',
    body: 'Sovnik axúdache a aprender español, portugués, galego, catalán, éuscaro e máis con probas de vocabulario e aplicacións móbiles.',
    cta: 'Descubre Sovnik',
  },
  map: {
    legendTitle: 'Fiabilidade das estacións',
    legendHigh: 'Alta',
    legendMid: 'Media',
    legendLow: 'Baixa',
    legendNoData: 'Sen datos',
    legendAirport: 'Aeroporto',
    legendVisited: 'Visitada',
    hideVisitedA11y: 'Ocultar estacións visitadas',
    showAllStationsA11y: 'Amosar todas as estacións',
    openStation: 'Abrir estación',
    openDetails: 'Detalles',
    flightsFromIberia: 'Voos desde a Península Ibérica',
    noIberianFlights: 'Sen voos ibéricos na última mostra.',
    locateA11y: 'Centrar o mapa na miña localización',
    shareA11y: 'Compartir imaxe do mapa',
    closeA11y: 'Pechar tarxeta da estación',
    shareFailedTitle: 'Non se puido compartir',
    shareFailedBody: 'Algo saíu mal ao preparar a imaxe do mapa.',
    shareMap: 'Compartir mapa',
    sharingUnavailable: 'Compartir non está dispoñible neste dispositivo.',
  },
  rankings: {
    title: 'Rankings',
    subtitle: 'Fiabilidade e votos da comunidade en estacións e hoteis.',
    mostReliable: 'Top 7 máis fiábeis (Portugal)',
    leastReliable: 'Bottom 7 menos fiábeis (Portugal)',
    spainMostReliable: 'Top 7 máis fiábeis (España)',
    spainLeastReliable: 'Bottom 7 menos fiábeis (España)',
    topUpvotedStations: 'Estacións máis votadas positivamente',
    mostDownvotedStations: 'Estacións máis votadas negativamente',
    topUpvotedHotels: 'Hoteis máis votados positivamente',
    mostDownvotedHotels: 'Hoteis máis votados negativamente',
    votesUnavailable: 'Os votos da comunidade non están dispoñibles temporalmente.',
    trainSpotlightTitle: 'Destacados de trens (Portugal)',
    mostReliableTrain: 'Os 3 trens máis fiábeis',
    mostDelayedTrain: 'Os 3 trens máis atrasados',
    trainSpotlightAvgDelay: '{{avg}} min de atraso medio',
    trainSpotlightMajorStations: 'Vía',
    stationHighlightTitle: 'Estación para explorar',
    stationHighlightIntro:
      'Unha parada ferroviaria ibérica ao chou da guía, coa descrición curta e a fiabilidade cando temos mostras dabondo.',
    stationHighlightIntroPt:
      'Unha parada ferroviaria portuguesa ao chou da guía, coa descrición curta e a fiabilidade cando temos mostras dabondo.',
    stationHighlightIntroEs:
      'Unha parada ferroviaria española ao chou da guía, coa descrición curta e a fiabilidade cando temos mostras dabondo.',
    stationHighlightCta: 'Abrir páxina da estación',
    stationHighlightSamples_one: '{{count}} observación nas nosas mostras',
    stationHighlightSamples_other: '{{count}} observacións nas nosas mostras',
  },
  tickets: {
    title: 'Billetes',
    howToBuy: 'Como mercar',
    serviceTypes: 'Tipos de servizo',
    usefulLinks: 'Ligazóns útiles',
    open: 'Abrir',
    proTitle: 'VeryStays Pro',
    proBody: 'Desbloquea Pro cunha subscrición mensual. Xestiona o teu plan en calquera momento desde aquí.',
    proCta: 'Ver plans Pro',
    proActiveTitle: 'Tes Pro',
    proActiveBody: 'Grazas por apoiar VeryStays. Abre o paywall en calquera momento para revisar o teu plan.',
    proManage: 'Xestionar subscrición',
    proUnavailable: 'Compras non dispoñibles',
    privacyPolicy: 'Política de privacidade',
  },
  hotel: {
    kmFromStation: '{{km}} km da estación',
    fromPerNight: 'desde €{{price}}/noite',
    view: 'Ver',
  },
  onboarding: {
    welcomeEyebrow: 'Benvido',
    welcomeTitle: 'Estacións ibéricas no peto',
    welcomeBody:
      'Explora hubs de Portugal e España, segue un tren e mantén contas atrás na pantalla de bloqueo e na de inicio.',
    welcomeBodyAndroid:
      'Explora hubs de Portugal e España, consulta saídas en directo e segue o tren que vas coller.',
    welcomeBullet1: 'Atopa estacións e saídas en directo',
    welcomeBullet2: 'Segue unha viaxe con Live Activity',
    welcomeBullet2Android: 'Segue unha saída na app',
    welcomeBullet3: 'Engade o widget Conta atrás do tren',
    welcomeBullet3Android: 'Ve estacións ordenadas preto de ti',
    locationEyebrow: 'Localización',
    locationTitle: 'Atopa estacións preto de ti',
    locationBody:
      'A localización é opcional. Ordena estacións por distancia, alimenta o widget da estación máis próxima e pode avisarte cando chegas a grandes hubs. Sempre funciona mellor en segundo plano; Mentres usas a app aínda axuda coa app aberta.',
    locationBodyAndroid:
      'A localización é opcional, pero permite a VeryStays ordenar estacións por distancia na pantalla de inicio.',
    notificationsEyebrow: 'Notificacións',
    notificationsTitle: 'Recordatorios suaves de viaxe',
    notificationsBody:
      'Cando tocas Coller nunha saída, podemos lembrarche minutos antes de que saia o tren. A localización tamén pode avisarte cando chegas a estacións concorridas — Sempre funciona mellor coa app pechada.',
    widgetsEyebrow: 'Dun vistazo',
    widgetsTitle: 'Widgets e Live Activity',
    widgetsTitleAndroid: 'Conta atrás da viaxe',
    widgetsBody:
      'Engade o widget Conta atrás do tren á pantalla de inicio. Ao seguir unha viaxe, tamén pode aparecer unha Live Activity na pantalla de bloqueo e na Dynamic Island.',
    widgetsBodyAndroid:
      'Cando premas Vou coller nunha saída, VeryStays mantén unha conta atrás da viaxe para saíres a tempo cara á estación.',
    widgetsHint:
      'Mantén premida a pantalla de inicio, toca +, busca VeryStays e escolle Conta atrás do tren.',
    widgetsHintAndroid:
      'Abre unha estación con saídas en directo e preme Vou coller no tren que queiras.',
    enableLocation: 'Continuar',
    enableNotifications: 'Continuar',
    finish: 'Comezar',
    notificationsFooter: 'Opcional. Podes cambiar as notificacións máis tarde en Axustes.',
    locationFooter: 'Opcional. Podes activar a localización máis tarde en Axustes.',
  },
  engagement: {
    channelName: 'Consellos e exploración',
    mapWanderTitle: 'Algo novo no mapa',
    mapWanderBody:
      'Abre o mapa e percorre unha liña que aínda non fixeches — as estacións esconden as mellores escapadas dun día.',
    liveCountdownTitle: 'Non perdas o andeén',
    liveCountdownBody:
      'Preme Vou coller nunha saída e leva a conta atrás contigo — pantalla de bloqueo, Dynamic Island e widgets.',
    budgetStaysTitle: 'Queda xunto ás vías',
    budgetStaysBody:
      'Explora aloxamentos económicos a pé das estacións en Portugal e España.',
    reliabilityTitle: 'Viaxa con mellores previsións',
    reliabilityBody:
      'Consulta a fiabilidade de puntualidade antes de ir e escolle a estación que encaixe no teu día.',
    visitedTitle: 'Constrúe o teu mapa de viaxes',
    visitedBody:
      'Marca estacións como visitadas e mira medrar a túa rede ibérica parada a parada.',
    nearestTitle: 'Que hai ao teu arredor?',
    nearestBody:
      'Ordena por distancia e explora os hubs máis próximos — a próxima aventura pode estar a un salto.',
    airportTitle: 'Do aeroporto á cidade, ordenado',
    airportBody:
      'As páxinas de aeroporto mostran ligazóns de destino para planear o primeiro tren despois de aterrar.',
    widgetTitle: 'O teu tren dun vistazo',
    widgetBody:
      'Engade o widget Conta atrás do tren á pantalla de inicio e mantén a próxima saída á man.',
  },
  notFound: {
    title: 'Ups!',
    message: 'Esta pantalla non existe.',
    home: 'Ir ao inicio',
  },
  tripNotify: {
    title: 'Tren a piques de saír',
    body: 'Agardamos que chegases ao tren das {{time}} que te leva de {{origin}} a {{destination}}.',
    channelName: 'Recordatorios de viaxe',
    departedTitle: 'O tren saíu',
    departedBody: 'Boa viaxe de {{origin}} a {{destination}}.',
  },
  arrivalNotify: {
    title: 'Estás na estación',
    body: 'Benvido a {{station}}. Abre VeryStays para saídas en directo.',
    airportTitle: 'Estás no aeroporto',
    airportBody: 'Benvido a {{station}}. Abre VeryStays para ligazóns e saídas.',
    channelName: 'Chegadas á estación',
  },
};
