...
Station and hotel data live in `src/data/` as TypeScript files. CP station codes are in `src/data/cpStationCodes.ts` (regenerate with `node scripts/map-cp-stations.mjs`). Reliability scores are collected offline (`npm run stats:departures`) and published to `public/data/reliability-scores.json`. The same command also refreshes airport connection maps, appends a private station temperature log from [Open-Meteo](https://open-meteo.com/) for train stations that returned a departure sample and for airport hubs that got a successful flight sample (`data/station-temperature-log.ndjson`, not shown in the UI), appends per-train arrival delay samples (`data/train-delay-log.ndjson`, not shown in the UI), and syncs `mobile/data/`.

**Catalog expansion** grows the Iberian set (PT/ES stations, Iberian airports, metro termini) toward a soft cap of **1411**, adding **at most one** Spanish or Portuguese station per `stats:departures` run (never bulk-fills to the cap). Pass `--hold-catalog-expand` to freeze, `--skip-catalog-expand` to skip one run, or `--spain-expand` / `--portugal-expand` to prefer one country. Europe destination upserts and new compact external-airport pages stay opt-in (`--expand-europe-destinations`, `--expand-external-airport-pages`). Reliability sampling and connection-map redraws always run.

Airport destination maps are sampled from AviationStack (AirLabs Schedules, then AeroDataBox RapidAPI as quota fallbacks) and baked into `public/data/airport-connections.json` plus `public/maps/airports/*-connections.png`. They roll on a fixed calendar (Europe/Lisbon): **1 Jan, 7 Feb, 16 Mar, 22 Apr, 29 May, 5 Jul, 11 Aug, 17 Sep, 24 Oct** (same dates each year). On each open date the previous period is frozen under `public/data/airport-connections/periods/{YYYY-MM-DD}.json` and `public/maps/airports/periods/{YYYY-MM-DD}/`, then a new live period starts. Check status with `npm run maps:airport-connections -- --period-status`. Site/app always show the live (current period) paths. Set `AVIATIONSTACK_API_KEY`, `AIRLABS_API_KEY`, and/or `AERODATABOX_RAPIDAPI_KEY` in `.env`.
...
**Europe destination airports:** connection collection can upsert European destinations into `src/data/europe/airports.ts` as `Airport Destination` stations (map only; never departure hubs). **That upsert is frozen by default** — pass `--expand-europe-destinations` on a connections run, or backfill explicitly:

```bash
npm run maps:airport-connections -- --backfill-europe-destinations
```

Those stations appear on the map (and under the Airport filter on `/all`) but are **never** used as departure hubs — `loadAirportCatalog` only reads PT/ES. Non-mainland ES/PT (Canaries, Balearics, Azores, Madeira, Ceuta, Melilla) can become destinations; mainland Iberia hubs stay hub-only.
Each `npm run stats:departures` adds **one Iberian-flights-only map** outside the peninsula (`public/maps/airports/external/{iata}-{place}-iberia.png`, no flight API). When the 5-hour airport-hub cooldown has elapsed, the same run also samples **one all-flights map** (`{iata}-{place}.png`) from a flight API. Both files are kept. Compact station pages for those airports stay at the current IATA list unless you pass `--expand-external-airport-pages`. The running map list is stored in `data/external-airport-connection-maps.json` and printed at the end of `stats:departures`. To draw several Iberian maps at once from hub data we already have:
...
npm run images:ensure                          # public pages only; used by stats:departures + mobile builds
npm run images:ensure:mobile                   # same, then sync mobile/data
