...
Station and hotel data live in `src/data/` as TypeScript files. CP station codes are in `src/data/cpStationCodes.ts` (regenerate with `node scripts/map-cp-stations.mjs`). Reliability scores are collected offline (`npm run stats:departures`) and published to `public/data/reliability-scores.json`. The same command also refreshes airport connection maps, appends a private station temperature log from [Open-Meteo](https://open-meteo.com/) for train stations that returned a departure sample and for airport hubs that got a successful flight sample (`data/station-temperature-log.ndjson`, not shown in the UI), appends per-train arrival delay samples (`data/train-delay-log.ndjson`, not shown in the UI), adds **up to three unmatched Spanish stations** and **one unmatched Portuguese CP halt** from GTFS when there are still stops missing from the catalog (`--skip-spain-expand` / `--skip-portugal-expand` to skip), generates surrounding-area maps for newly added Spanish stations on **about 10%** of expand runs (no backfill of older missing maps), and syncs `mobile/data/`.
...
Each `npm run stats:departures` adds **one Iberian-flights-only map** outside the peninsula (`public/maps/airports/external/{iata}-{place}-iberia.png`, no flight API). When the 5-hour airport-hub cooldown has elapsed, the same run also samples **one all-flights map** (`{iata}-{place}.png`) from a flight API. Both files are kept. Airports with both maps get a compact station page (photo, hotels, eSIM, surrounding-area map); the rest stay off station lists. The running list is stored in `data/external-airport-connection-maps.json` and printed at the end of `stats:departures`. To draw several Iberian maps at once from hub data we already have:
