...
**Catalog expansion is on hold** at the current ~1404 Iberian hubs/stops (plus the existing Europe destination / compact-page set). Departures still collect reliability, redraw hub connection maps when destinations change, and draw both kinds of external destination maps when data is available — but it does **not** add Spanish/Portuguese stations, upsert new Europe destination airports, or open new compact airport pages unless you opt in (`--spain-expand`, `--portugal-expand`, `--expand-europe-destinations`, `--expand-external-airport-pages`).
