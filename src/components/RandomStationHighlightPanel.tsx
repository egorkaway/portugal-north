...
export function RandomStationHighlightPanel() {
  ...
  useEffect(() => {
    ...
    if (portugalQuery.isLoading || spainQuery.isLoading) return;
    pickedRef.current = true;
    setStation(
      pickRandomStationHighlight(pageStations, {
  ...
  return (
    <section
      aria-labelledby="station-highlight-heading"
      className="mt-8 mb-2 border-t border-border pt-8 md:mt-12 md:pt-10"
    >
      <div className="mb-3 md:mb-4">
        <h2 id="station-highlight-heading" className="font-display text-2xl text-foreground">
          {t("rankings.stationHighlightTitle")}
        </h2>
        <p className="mt-1 text-sm text-muted-foreground">{t("rankings.stationHighlightIntro")}</p>
      </div>
