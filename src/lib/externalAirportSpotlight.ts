...
/**
 * Directory gitignore: keep Iberian-only maps local until the airport has both
 * maps (and a public page). Page-airport Iberian PNGs stay tracked.
 */
export function externalIberiaMapsGitignore(pageIatas: readonly string[]): string {
  const iatas = [
    ...new Set(
      pageIatas
        .map((iata) => iata.trim().toLowerCase())
        .filter((iata) => /^[a-z]{3}$/.test(iata)),
    ),
  ].sort();
  const lines = [
    "# Auto-generated by scripts/lib/externalAirportConnectionMaps.mjs — do not edit.",
    "# Iberian-only maps stay local until the airport has both maps (public page).",
    "*-iberia.png",
  ];
  for (const iata of iatas) {
    lines.push(`!${iata}-*-iberia.png`);
  }
  return `${lines.join("\n")}\n`;
}
