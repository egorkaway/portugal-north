 ...
 * Compact station pages exist only once both maps are present, and new pages
 * stay frozen unless `--expand-external-airport-pages` is passed.
 */
...
export function saveExternalAirportMapsStore(rootDir, store, options = {}) {
  const outPath = externalStorePath(rootDir);
  mkdirSync(dirname(outPath), { recursive: true });
  writeFileSync(outPath, `${JSON.stringify(store, null, 2)}\n`);
  writeExternalAirportPageIatas(rootDir, store, options);
  return outPath;
}

function readExistingExternalAirportPageIatas(rootDir) {
  try {
    const raw = readFileSync(join(rootDir, PAGE_IATAS_REL), "utf8");
    const match = raw.match(/EXTERNAL_AIRPORT_PAGE_IATAS[^=]*=\s*(\[[\s\S]*?\])/);
    if (!match) return [];
    const parsed = JSON.parse(match[1]);
    return Array.isArray(parsed)
      ? parsed.map((iata) => String(iata).trim().toUpperCase()).filter(Boolean)
      : [];
  } catch {
    return [];
  }
...
/**
 * Sync compact-page IATA list from airports that have both maps.
 * Catalog expansion is frozen by default — new both-map airports keep their
 * PNGs/store rows but do not get site/app pages until `expandPages: true`.
 */
export function writeExternalAirportPageIatas(rootDir, store, options = {}) {
  const expandPages =
    options.expandPages === true ||
    (typeof process !== "undefined" &&
      Array.isArray(process.argv) &&
      process.argv.includes("--expand-external-airport-pages"));
  const fromStore = (store?.airports ?? [])
    .map((row) => normalizeExternalAirportRow(row))
    .filter((row) => hasBothExternalMaps(row))
    .map((row) => String(row.iata ?? "").trim().toUpperCase())
    .filter(Boolean)
    .sort();
  const existing = readExistingExternalAirportPageIatas(rootDir);
  const unique = expandPages
    ? [...new Set(fromStore)]
    : [...new Set(existing)].sort();
  if (!expandPages) {
    const skipped = fromStore.filter((iata) => !unique.includes(iata));
    if (skipped.length) {
      console.log(
        `External airport pages frozen — not adding ${skipped.join(", ")} (pass --expand-external-airport-pages).`,
      );
    }
  }
